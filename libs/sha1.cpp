...
/// process 64 bytes
void SHA1I::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void SHA1I::add(const void* data, size_t numBytes)
{
...
void SHA1I::getHash(unsigned char buffer[SHA1I::HashBytes])
{
...
/// compute SHA1I of a memory block
std::string SHA1I::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
/// compute SHA1I of a string, excluding final zero
std::string SHA1I::operator()(const std::string& text)
{
  reset();
